...
def classwise_f1(output, gt):
    ...
    true_positives = torch.tensor(((output == 1) * (gt == 1)).sum()).float()
    selected = torch.tensor((output == 1).sum()).float()
    relevant = torch.tensor((gt == 1).sum()).float()

    precision = (true_positives + EPSILON) / (selected + EPSILON)
    recall = (true_positives + EPSILON) / (relevant + EPSILON)
    classwise_f1 = 2 * (precision * recall) / (precision + recall)

    specificity = torch.tensor(((output == 0) * (gt == 0)).sum()).float() / (torch.tensor((gt == 0).sum()).float() + EPSILON)
    sensitivity = recall

    return classwise_f1, (specificity + sensitivity) / 2
